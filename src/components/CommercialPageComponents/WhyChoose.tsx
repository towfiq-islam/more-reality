...
const WhyChoose = () => {
    return (
        <div className="container py-20 grid grid-cols-2 gap-5 items-center">
            ...
            {/* Right */}
            <div>
                <h3 className="text-2xl font-bold mb-10 -ml-5">Why Choose MORE Realty for Commercial Deals</h3>
                <ol className="list-decimal list-outside space-y-7 marker:text-xl marker:font-bold">
                    {data.map(item => (
                        <li key={item.id}>
                            <div className="pl-2">
                                <span className="font-bold text-xl">{item.title}: </span>
                                <span className="text-secondary-text leading-[30px] text-[19px]">{item.desc}</span>
                            </div>
                        </li>
                    ))}
                </ol>
